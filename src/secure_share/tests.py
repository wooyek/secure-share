...
class SampleDetailTests(UserTestCase):
    def test_anonymous(self):
        ...
        url = resolve_url("secure_share:SampleDetail", item.pk)
...
class SampleCreateTests(UserTestCase):
    def test_anonymous(self):
        url = resolve_url('secure_share:SampleCreate')
        response = test.Client().get(url)
        self.assertEqual(302, response.status_code)
    ...
    def test_post(self):
        ...
        self.assertRedirects(response, resolve_url('secure_share:SampleDetail', item.pk), fetch_redirect_response=False)
